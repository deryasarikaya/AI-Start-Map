...
def test_the_prompt_says_what_the_knowledge_is_and_is_not() -> None:
    ...
    prompt = (
        Path(__file__).resolve().parents[1] / "app/prompts/ergebnis_teil1.md"
    ).read_text(encoding="utf-8")

    assert "NUR_INTERNES_VERGLEICHSWISSEN_NIE_AUSGEBEN" in prompt
    assert "keine Beschreibung dieses Betriebs" in prompt
    assert "Übernimm\nkeinen Wortlaut daraus" in prompt
    assert "Der Abschnitt ist oft leer" in prompt
