...
def test_model_prompt_context_contains_no_internal_metadata() -> None:
    ...
    forbidden_markers = (
        ...
        "pattern_id",
        "content_origin",
        "ten_cases_rag_corpus.md",
        "additional_kmu_rag_corpus.md",
    )
    assert all(
...
@pytest.mark.parametrize(
    "demo_slug", ["etsy-3d-print", "carpet-cleaning"]
)
def test_demo_route_creates_session_and_reaches_the_result(
    ...
    for kennung in ("c-01", "k-01", "testfall", "chunk", "pattern_id",
                    "content_origin"):
...
def test_a_failing_second_call_shows_an_error_and_stores_nothing(
    ...
    **Und der Warteschirm erfährt den Grund sofort.** Er stand vorher auf
    ...
    assert client.get("/demo/etsy-3d-print", follow_redirects=False).status_code == 303
