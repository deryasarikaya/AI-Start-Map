"""Die Form der Modellantworten — und die Wächter davor.

Zwei Aufgaben in einer Datei:

**Form.** Beschreibt, welche Felder eine Antwort des Modells haben muss. Was
nicht hineinpasst, wird abgewiesen statt notdürftig repariert.

**Wächter.** Eine Reihe von Prüfungen, die verhindern, dass etwas nach außen
dringt, das dort nicht hingehört — interne Kennungen wie Musternummern,
Dateinamen aus dem Wissensbestand, Fachsprache, und Versprechen über Zeit- oder
Geldersparnis, die niemand belegen kann.

Diese Wächter sind aus konkreten Fehlern entstanden. Wer eine Prüfung
lockert, sollte wissen, welchen Fall sie einmal verhindert hat.
"""
...
def _ensure_direct_customer_language(
    ...
    *,
    ...
    """Sorgt dafür, dass der Text den Kunden direkt anspricht."""
...
#: damit das Modell genau die Liste sieht, die auch geprüft wird. Früher
#: standen beide Listen getrennt und liefen auseinander: der Filter kannte
#: Wörter, die der Prompt nie genannt hat, und verwarf dann fertige Analysen.
...
FORBIDDEN_CUSTOMER_TERM_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    # Aus den Live-Läufen Blumenladen, Fotograf und Handwerk. Geprüft wird der
    ...
    re.compile(
        ...
        # anderes als das, was tatsächlich passiert.
        r"\b(?:an)?trainier\w*\b|\banlern\w*\b|\bLernphase\w*\b",
        re.IGNORECASE,
    ),
    # Schreibungsabhängig: als deutsches Substantiv groß geschrieben.
    # Kleingeschrieben steckt es in CSS-Klassennamen wie "result-layout",
    # die beim Prüfen des gerenderten HTML sonst falsch anschlagen.
...
def contains_internal_reference(value: Any) -> bool:
    """Ob irgendwo eine interne Kennung steht.

    Fall-, Muster- oder Chunk-Bezeichnungen dürfen den Kunden nie
    erreichen.
    """
...
def contains_prohibited_customer_language(value: Any) -> bool:
    """Ob der Text Formulierungen enthält, die nicht rausgehen dürfen."""
...
def _neutralize_internal_reference_fields(value: Any) -> tuple[Any, bool]:
    """Entfernt interne Kennungen und meldet, ob etwas entfernt wurde."""
...
class StrictResultModel(BaseModel):
    ...
    def reject_internal_references(self) -> StrictResultModel:
        """Weist eine Modellantwort mit internen Kennungen zurück."""
...
class ProcessSuggestion(StrictResultModel):
    ...
    def reject_general_category(self) -> ProcessSuggestion:
        """Weist allgemeine Kategorien statt konkreter Abläufe zurück.

    'Marketing' oder 'Organisation' ist kein Ablauf mit Beginn und Ende.
        """
...
class FollowUpQuestion(StrictResultModel):
    ...
    def validate_current_process_question(self) -> FollowUpQuestion:
        """Prüft, dass die Rückfrage nach dem heutigen Ablauf fragt."""
...
class FollowUpResult(StrictResultModel):
    ...
    def validate_unique_questions(self) -> FollowUpResult:
        """Prüft, dass keine Rückfrage doppelt vorkommt."""
...
def _validate_generated_heading(value: str, field_name: str) -> str:
    ...
    text = value.strip()
    # Zusammengesetzte Wörter mit Bindestrich zählen als eines:
...
#: Wörter, die in "das fehlt noch" nichts über den Inhalt aussagen und
#: deshalb beim Abgleich mit der Nachricht nicht mitzählen.
...
class Beispiel(StrictResultModel):
    ...
    def check_titel(cls, value: str) -> str:
        """Prüft die Überschrift auf Länge und Form."""

        return _validate_generated_heading(value, "beispiel.titel")

    @model_validator(mode="after")
    def check_beispiel(self) -> Beispiel:
        """Prüft, dass die Veranschaulichung zur gewählten Darstellungsart passt."""
        ...
        gefuellt = set(labels)
        for entry in self.fehlt:
            if entry.casefold().strip() in gefuellt:
                raise ValueError(
        ...
        # fragte die Rückfrage nach dem Bonfoto, obwohl "Foto angehängt" in
        # der Nachricht stand. Geprüft werden die Inhaltswörter des Eintrags.
        nachricht = self.nachricht.casefold()
        for entry in self.fehlt:
            woerter = [
                wort
                for wort in re.findall(r"[a-zäöüß]{4,}", entry.casefold())
                if wort not in _FEHLT_FUELLWOERTER
            ]
            if woerter and all(wort in nachricht for wort in woerter):
                raise ValueError(
                    f"Als fehlend markiert, steht aber in der Nachricht: {entry!r}"
                )
        return self
...
class Loesung(StrictResultModel):
    titel: Annotated[str, Field(min_length=1, max_length=120)]
    # Bewusst weit über dem, was je gebraucht wird. Bei 180 schrieb das Modell
    # bis genau an die Grenze und brach mitten im Wort ab - ein abgeschnittener
    # Satz ist für den Kunden schlimmer als ein langer. Die Kürze kommt aus
    # dem Prompt (höchstens fünfzehn Wörter je Schritt), nicht von hier.
    ablauf_heute: list[
    ...
    def check_ablauf(self) -> Loesung:
        """Prüft, dass der künftige Ablauf vollständig beschrieben ist."""
...
class Voraussetzungen(StrictResultModel):
    ...
    def check_kein_ueberschneiden(self) -> Voraussetzungen:
        ...
        added = {normalize(item) for item in self.neu_hinzukommend}
        if bleibt & added:
            raise ValueError(
...
class Umsetzung(StrictResultModel):
    ...
    def check_umsetzung(self) -> Umsetzung:
        """Prüft die Angaben zur Umsetzung."""
...
#: dem Wortfilter. Ist-Ablauf, Diagnose und die aus dem Katalog gefüllten
...
class FinalAnalysisResult(StrictResultModel):
    ...
    #: Wird nicht als eigener Block gezeigt - steckt in möglichkeiten mit Rang
    #: "später". Doppelt zeigen ist laut Spec ein Fehler.
    spaeter: list[
        Annotated[str, Field(min_length=1, max_length=220)]
    ] = Field(default_factory=list, max_length=3)

    # --- Ist-Ablauf und Diagnose: unverändert --------------------------------
    ...
    def check_engpass_titel(cls, value: str) -> str:
        """Prüft die Überschrift des Engpasses."""
    ...
    def repair_direct_bleibt_bei_dir(cls, value: Any) -> Any:
        """Formt 'Das bleibt bei dir' in die direkte Anrede um."""
    ...
    def validate_customer_output(self) -> FinalAnalysisResult:
        """Die Schlussprüfung über den gesamten Kundentext.

    Hier fällt durch, was der Kunde nie sehen darf - unabhängig davon,
    welches Feld es betrifft.
        """
