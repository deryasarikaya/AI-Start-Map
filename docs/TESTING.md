...
| **Die Vollständigkeit** | `test_presentation_completeness.py` | Anker belegt, Einstieg beantwortet was er bringt, Abhängigkeiten und Kanten kuratiert — geprüft am eingecheckten Vorführfall |
...
| **Die Seiten** | `test_example_page.py`, `test_report_page.py`, `test_ux_journey.py`, `test_results_v1_renderer.py`, `test_karte.py` | Ergebnisseite, PDF, Rückfallebene sowie DTO-only Results-V1 mit vierstufiger Betriebslandkarte, Alltagsflow, konkreter Experience-Lösungsvorschau, Verantwortung, Grenzen und Future-Features |
